...
class AppWindow:
    ...
    def _detect_invisible_object(self):
        ...
        gt_6d_pose_data_copy = deepcopy(gt_6d_pose_data)
        ...
        for result in results:
            for i in range(len(result[1]) - 1, -1, -1):
                if result[1][i] == 0:
                    gt_6d_pose_data_copy[str(result[0])].remove(gt_6d_pose_data_copy[str(result[0])][i])

        with open(json_6d_path, "w+") as gt_scene:
            json.dump(gt_6d_pose_data_copy, gt_scene)
